...
class Rocket(TestbedBase):
    ...
    def __init__(self, settings=None): 
        ...
        # home planet
        home_planet = self.world.create_kinematic_body(position=(10,0),
        ...
        # target planet
        target_planet = self.world.create_kinematic_body(position=(100,100),
            fixtures=b2d.fixture_def(
        ...
        # black hole
        black_hole = self.world.create_kinematic_body(position=(0,400),
            fixtures=b2d.fixture_def(
        ...
        # a tiny rocket
        self.rocket = self.world.create_dynamic_body(position=(10,10), 
            fixtures=[
                b2d.fixture_def(
                ...
                b2d.fixture_def(
                    shape=b2d.polygon_shape(vertices=[
                        (-1, 1), (0, 4), (1,1)
                    ]),
                    density=1
                )
        ...
        emitter_def.transform = b2d.Transform(world_anchor, b2d.Rot(self.emitter_local_rot[0]))
        ...
        emitter_def.transform = b2d.Transform(world_anchor, b2d.Rot(self.emitter_local_rot[1]))
        emitter = b2d.RandomizedRadialEmitter(psystem, emitter_def)
        self.emitters.append(emitter)
    ...
    def pre_step(self, dt):
        ...
        if  self.touched_black_hole:
            ...
            for planet,planet_def in self.planets.items():
                ...
                f = -self.gravitational_constant * rocket_mass * planet_mass / (distance*distance)
                net_force += delta * f
            ...
            for emitter,local_anchor,local_rotation in  zip(self.emitters,self.emitter_local_anchors, self.emitter_local_rot):
    ...
    def post_debug_draw(self):
        for planet, planet_def in self.planets.items():
            ...
            self.debug_draw.draw_solid_circle(pos, planet_def['radius']+0.1,axis=None, color=planet_def['color'])
            if planet.user_data == "black_hole":
                self.debug_draw.draw_circle(pos, planet_def['radius']*5, color=(1,1,1), line_width=0.1)
